...
export class DataService {
  ...
  public fetchSimulationResults(startMatrix: Array<Array<number>>, ruleName: string) {
    ...
    if (simType == SimulationType.Block) {
      ...
      return this.http.post(this.url + "simulateBlock", new BlockStartConditions(startMatrix, rulesSet.Rules, SimulationSettings.StepsPerRequest));
    }
    const rulesSet = this.rulesService.getMooreRulesSet(ruleName);
    return this.http.post(this.url + "simulateMoore", new MooreStartConditions(startMatrix, rulesSet.Rules, SimulationSettings.StepsPerRequest, 0));
  }
